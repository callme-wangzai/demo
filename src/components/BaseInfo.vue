  ...
  <div class="BaseInfo">
    ...
    <van-form @submit="onSubmit">
      <van-field
        ...
        label="姓名"
        placeholder="请输入姓名"
        :rules="[{ required: true, message: '' },{validator:validatorName,message: '姓名只能为汉字'}]"
      />
      ...
      <van-field
        ...
        label="年龄"
        placeholder="请输入年龄"
        :rules="[{ required: true, message: '' },{validator,message: '年龄应大于18岁，小于70岁'}]"
      />
      <van-field
        is-link
        required
      ...
      <van-field
        ...
        label="手机号码"
        placeholder="请输入手机号码"
        :rules="[{ required: true, message: '' },{ pattern, message: '请输入正确手机号' }]"
      />
      <!-- <van-field
        v-model="smsCode"
        label-width="80px"
      ...
      <van-field
        ...
        :rules="[{ required: true, message: '' },{validator:validatorMount,message: '额度应大于5000，小于500万'}]"
      />
    ...
    <van-popup v-model="show" position="bottom" :style="{ height: '46%' }">
        <van-area @cancel="cancelFn" :area-list="areaList" @confirm="confirmFn"
          columns-num="2" value="510100" title="选择所在省市" />
    </van-popup>
    <div class="tips">
      <!-- <div>声明：</div> -->
...
export default {
  ...
  methods:{
    ...
    getSign(){
      ...
      axios.get(`https://web.rongxinvip.com/weixin/getAppSign?code=${this.uid}`)
      .then(res=>{
        if(res.data.code===0){
    ...
    sendMsg(){
      ...
      .then(res=>{
        if(res.data.code===0){
          ...
          instance.post('https://web.rongxinvip.com/weixin/sms/sendSmsCode',{
            phone:this.phone
          })
          .then(res=>{

<template>
  <div class="Result">
    ...
    <div v-show="$route.query.success==='1'" class="message_success">系统审核通过</div>
    <img v-show="!$route.query.success||$route.query.success==='2'" width="50px" :src="deposit_wrong" alt="">
    <div v-show="!$route.query.success||$route.query.success==='2'" class="message_wrong">系统审核未通过</div>
    <div class="name">{{$route.query.clientName}}</div>
    <div class="phone">{{$route.query.phone}}</div>
    <div v-show="$route.query.success==='1'" class="tips">稍后将有审核专员电话回访，请保持电话畅通</div>
    <div v-show="!$route.query.success||$route.query.success==='2'" class="tips">请过段时间再尝试申请</div>

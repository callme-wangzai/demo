    ...
    <div v-if="!loadFlag" class="AssetInfo">
      ...
      <van-form @submit="onSubmit">
        <van-field
          ...
          :rules="[{ required: true, message: '' },{ validator, message: '请输入正确的分数'}]"
        />
        <van-field
          ...
          :rules="[{ validator:validatorMonth, message: '社保不可超过120个月'}]"
        />
        <van-field
        ...
        <van-field
          ...
          :rules="[{ validator:validatorHouse, message: '房产最高1000万，最低20万'}]"
        />
        <van-field
          v-model="carValue"
          type="number"
        ...
        /><van-field
          ...
          :rules="[{ validator:validatorPerson, message: '个人收入最高100万'}]"
        />
...
export default {
  ...
  methods:{
    ...
    onSubmit(values) {
      ...
      if(!this.$route.query.employeeNumber||this.$route.query.employeeNumber<1000||this.$route.query.employeeNumber>99999){
      ...
      // instance.post('https://web.rongxinvip.com/weixin/sms/checkSmsCode',{
      ...
      //       return
      //     }
      //     obj.verifyCode=obj.smsCode
          delete obj.smsCode
          delete obj.sign
          instance.post('https://web.rongxinvip.com/weixin/add',obj)
          .then(res=>{
            if(res.data.code===0&&res.data.data!==0){//成功有额度
              this.loadFlag=true
              setTimeout(res=>{
                this.loadFlag=false
                console.log('res',res)
            ...
            }else if(res.data.code===0&&res.data.data===0){//成功无额度
              this.loadFlag=true
                setTimeout(res=>{
                  this.loadFlag=false
                  console.log('res',res)
